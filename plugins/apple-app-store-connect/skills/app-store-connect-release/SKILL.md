...
After any upload succeeds, poll App Store Connect builds until the new build is processed and `VALID`, make sure App Encryption Documentation is set to `None of the algorithms mentioned above` by running `configure-build-compliance --build-id <build-id> --yes` when the upload path did not already do it, run `verify-selected-build` against the artifact, then set `version.buildId` to that build ID and run `apply-metadata --yes` after confirmation. "Ready for submission" means the App Store Connect version has the uploaded build selected, the selected build has `usesNonExemptEncryption=false` and no `Missing Compliance` warning, required metadata applied, screenshots uploaded, subscription prices/trials configured, subscription availability verified, subscription review screenshots uploaded and verified from App Store Connect readback, first-time IAP/subscription products selected with the app version when required, review details complete, and preflight checks passing. A local archive, GitHub push, or unselected uploaded build is not ready for submission.
...
- Record `revenueCatIntegration.products[]` so every App Store subscription product maps to a RevenueCat product, package, and entitlement. Verify the offering is current and that the RevenueCat-backed paywall has restore purchases plus terms/privacy links visible before declaring release readiness.
